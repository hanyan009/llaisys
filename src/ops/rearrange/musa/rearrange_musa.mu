...
void rearrange(tensor_t out, tensor_t in) {
    ...
    int ndim = out->ndim();
    if (ndim > MAX_NDIM) {
        fprintf(stderr, "rearrange: ndim %d > MAX_NDIM %d\n", ndim, MAX_NDIM);
        return;
    }
    ...
    if (dtype == LLAISYS_DTYPE_F32) {
        ...
        rearrange_kernel<__mt_bfloat16><<<gridSize, blockSize>>>((__mt_bfloat16*)out->data(), (const __mt_bfloat16*)in->data(), meta);
    } else if (dtype == LLAISYS_DTYPE_I64) {
        rearrange_kernel<int64_t><<<gridSize, blockSize>>>((int64_t*)out->data(), (const int64_t*)in->data(), meta);
    } else {
         fprintf(stderr, "rearrange: unsupported dtype\n");
    }
    
    musaError_t err = musaGetLastError();
    if (err != musaSuccess) {
        fprintf(stderr, "Failed to launch rearrange kernel: %s\n", musaGetErrorString(err));
    }
}
