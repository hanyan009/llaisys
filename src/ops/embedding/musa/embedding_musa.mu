...
void embedding(tensor_t out, tensor_t index, tensor_t weight) {
    ...
    if (out->dtype() == LLAISYS_DTYPE_F32) {
        ...
        if (index->dtype() == LLAISYS_DTYPE_I64) {
             embedding_kernel<__mt_bfloat16, int64_t><<<blocks, threads>>>(
                (__mt_bfloat16*)out->data(), (const int64_t*)index->data(), (const __mt_bfloat16*)weight->data(),
                total_elements, hidden_size, vocab_size);
        } else if (index->dtype() == LLAISYS_DTYPE_I32) {
             embedding_kernel<__mt_bfloat16, int32_t><<<blocks, threads>>>(
                (__mt_bfloat16*)out->data(), (const int32_t*)index->data(), (const __mt_bfloat16*)weight->data(),
                total_elements, hidden_size, vocab_size);
        }
    }
