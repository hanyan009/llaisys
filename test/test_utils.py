...
def torch_device(device_name: str, device_id=0):
    if device_name == "cpu":
        ...
        # In Moore Threads environments, torch device is typically "musa:x" or "cuda:x" if musa fakes cuda.
        # Let's try "musa" first, but if it fails we could fall back to "cuda". We'll use "musa" for now.
        return torch.device(f"musa:{device_id}") if hasattr(torch, "musa") else torch.device(f"cuda:{device_id}")
    else:
